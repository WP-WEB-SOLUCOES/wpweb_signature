...
class _SignaturePageState extends State<SignaturePage> {
  ...
  int _authMethodIndex = 0; // 0: Rubrica, 1: Digitar, 2: OTP
  ...
  Future<void> _fetchIP() async {
    try {
      ...
      setState(() => _ipAddress = "PROTOCOLO_PROTEGIDO");
    }
  }
  ...
  Future<void> _fetchContract() async {
    ...
    try {
      final data = await Supabase.instance.client
          ...
          .select()
          .or('token_client.eq.$token,token_agency.eq.$token')
          .single();
      ...
      // SALVA NO CACHE PARA A HOME PAGE
      _saveToCache(data, token);

      // ATUALIZA NOME DA ABA
      SystemChrome.setApplicationSwitcherDescription(
        ApplicationSwitcherDescription(
          label: 'Contrato - ${data['client_name'] ?? "WP Web"}',
  ...
  Future<void> _saveToCache(Map<String, dynamic> data, String token) async {
    final prefs = await SharedPreferences.getInstance();
    final String? historyJson = prefs.getString('wpweb_history');
    List<dynamic> history = historyJson != null ? jsonDecode(historyJson) : [];

    history.removeWhere((item) => item['id'] == data['id']);
    history.insert(0, {
      'id': data['id'],
    ...
    await prefs.setString('wpweb_history', jsonEncode(history));
  }
  ...
  Future<void> _downloadSignedPDF() async {
    ...
    if (_contract!['agency_signature_url'] != null) {
      ...
      } catch (e) { debugPrint("Erro rubrica agência: $e"); }
    }
    ...
    pdf.addPage(pw.MultiPage(
      ...
      build: (context) => [
        ...
        if (isRevShare)
          _pdfClause("3. MODALIDADE DE PARCERIA (REVENUE SHARE)", "As partes pactuam regime de parceria técnica, onde a CONTRATADA investe capital intelectual em troca de ${revSharePercent.toStringAsFixed(1)}% de participação sobre o faturamento bruto das vendas/transações geradas pela plataforma."),

        _pdfClause("4. INVESTIMENTO", isRevShare 
          ? "O valor de ${currency.format(_contract!['total_value'])} refere-se ao SETUP DE INFRAESTRUTURA (Hostinger VPS/Cloud). O desenvolvimento intelectual é bonificado pela agência."
          : "Pela execução, o CONTRATANTE pagará à CONTRATADA o valor total de ${currency.format(_contract!['total_value'])}."),

        _pdfClause("5. CONFIDENCIALIDADE E PROPRIEDADE", "As partes obrigam-se ao sigilo absoluto. A titularidade dos fontes será cedida após a quitação do setup e início da operação."),
        _pdfClause("6. VALIDADE JURÍDICA", "Este documento possui validade plena nos termos da MP 2.200-2/2001, certificado via registro de IP e carimbo de tempo."),
  ...
  Widget build(BuildContext context) {
    ...
    if (_contract == null) return const Scaffold(body: Center(child: Text("Documento expirado ou link inválido.")));
            ...
            child: SingleChildScrollView(
              ...
              child: Center(
                child: Container(
                  ...
                  decoration: BoxDecoration(color: Colors.white, boxShadow: isMobile ? [] : [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 40)]),
  ...
  Widget _buildTopBar() {
    return Container(
      height: 65, color: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 20),
      child: Row(children: [
    ...
    return Column(children: [
      ...
      Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _clauseUI("1. IDENTIFICAÇÃO DAS PARTES", "Este contrato vincula juridicamente a CONTRATADA WP WEB SOLUÇÕES TECNOLÓGICAS (CNPJ 55.305.086/0001-52) e o CONTRATANTE $clientName, identificado via registro de IP e Token Único."),
          _clauseUI("2. DO OBJETO", "Desenvolvimento, arquitetura e licenciamento da solução técnica '${_contract!['project_title']}'."),
          _buildTechSpecs(),
          if (isRevShare) _clauseUI("3. DA PARCERIA E COMISSÃO", "O projeto será executado via Revenue Share. A CONTRATADA investe seu capital de desenvolvimento em troca de ${revSharePercent.toStringAsFixed(1)}% do faturamento bruto gerado pela plataforma."),
          _clauseUI(isRevShare ? "4. SETUP E INFRAESTRUTURA" : "3. INVESTIMENTO", isRevShare 
            ? "O CONTRATANTE pagará ${currency.format(_contract!['total_value'])} destinado a custos de Setup e Servidores. A mão de obra é investimento da agência."
            : "O CONTRATANTE pagará o montante de ${currency.format(_contract!['total_value'])} para execução integral."),
          _clauseUI("CONFIDENCIALIDADE", "As partes obrigam-se ao sigilo absoluto de segredos industriais e dados sensíveis."),
          _clauseUI("FORO", "Fica eleito o foro da comarca de Contagem/MG."),
  ...
  Widget _buildVisualHeader() {
    return Container(
      width: double.infinity, padding: const EdgeInsets.all(40), color: const Color(0xFF0F172A),
  ...
  Widget _buildPartnershipBadge() {
    return Container(
      margin: const EdgeInsets.only(bottom: 40), padding: const EdgeInsets.all(20),
  ...
  Widget _buildAuthPanel() {
    return Container(
      ...
      child: Column(children: [
        ...
        _buildTabs(),
        const SizedBox(height: 30),
        if (_authMethodIndex == 0) _buildPad() else if (_authMethodIndex == 1) _buildTyped() else _buildOtp(),
        const SizedBox(height: 30),
        _buildSubmitBtn(),
      ]),
    );
  ...
  Widget _buildTabs() {
    return Container(
      ...
      child: Row(children: [_tabItem(0, "Assinar"), _tabItem(1, "Digitar"), _tabItem(2, "OTP")]),
    );
  }
  ...
  Widget _buildSubmitBtn() {
    return Container(
      ...
      child: ElevatedButton(onPressed: _processSubmit, style: ElevatedButton.styleFrom(backgroundColor: Colors.transparent, shadowColor: Colors.transparent, foregroundColor: Colors.white), child: const Text("CONFIRMAR ASSINATURA", style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1))),
    );
  }
  ...
  Widget _buildOtp() => TextField(controller: _otpController, textAlign: TextAlign.center, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, letterSpacing: 8), decoration: const InputDecoration(hintText: "0000"));
